...
// ✅ Apni api file me ye function bana lena
import { signupUser } from "../api/api"; 

// Smooth Animation Hook (Same as Login)
...
export default function SignupScreen({ onSignup, onBack }: any) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // Backend se jo id aayegi wo App.tsx ko bhej denge
      const fetchedUserId = response.user_id || response.id;

      if (fetchedUserId) {
        onSignup(fetchedUserId); 
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.Text style={[styles.subtitle, { opacity: anim4.opacity, transform: [{ translateY: anim4.translateY }] }]}>
        Join FeedFlow AI and train your feed
      </Animated.Text>
      ...
      {/* Google Button (Optional but looks premium) */}
...
const styles = StyleSheet.create({
  ...
  logoContainer: {
    ...
    shadowOpacity: 0.5, shadowRadius: 25, elevation: 12, marginTop: 10, // Thoda upar kiya kyunki 3 inputs hain
  },
  ...
  subtitle: { color: "#94A3B8", textAlign: "center", marginTop: 10, marginBottom: 30, fontSize: 16, lineHeight: 24, paddingHorizontal: 20 },
  
  input: { 
    backgroundColor: "#1E293B", 
    padding: 16, // Thoda chota kiya kyunki 3 inputs hain
    borderRadius: 16, 
    marginBottom: 12, 
    color: "white", 
    fontSize: 16, 
